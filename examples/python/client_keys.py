...
def gateway_authentication():
    ...
    # Client only needs the gateway API key
    headers = {
...
def client_side_authentication():
    ...
    payload = {
        "model": "gpt-4o-mini",
        "messages": [
            {"role": "user", "content": "Hello with my own OpenAI key!"}
        ]
    }
    ...
    payload_gemini = {
        "model": "gemini-2.0-flash-exp",
        "messages": [
            {"role": "user", "content": "Hello with my own Gemini key!"}
        ]
    }
...
def hybrid_authentication():
    ...
    class HybridAuthClient:
        ...
        def make_request(self, payload: Dict[str, Any], provider: str = None) -> Optional[Dict[str, Any]]:
    ...
    test_cases = [
        {
            "payload": {
                "model": "gpt-4o-mini",
                "messages": [{"role": "user", "content": "Test hybrid auth with OpenAI"}]
            },
            "provider": "openai"
        },
        {
            "payload": {
                "model": "gemini-2.0-flash-exp",
                "messages": [{"role": "user", "content": "Test hybrid auth with Gemini"}]
            },
            "provider": "gemini"
        }
    ]
...
def dynamic_key_management():
    ...
    def test_with_active_key(provider: str, model: str):
        ...
        payload = {
            "model": model,
            "messages": [{"role": "user", "content": f"Test with {provider} using active key"}]
        }
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                print(f"✅ {provider} success: {data['choices'][0]['message']['content'][:50]}...")
            else:
...
def environment_key_configuration():
    ...
    if "gateway" in loaded_keys:
        ...
        payload = {
            "model": "gpt-4o-mini",
            "messages": [{"role": "user", "content": "Hello from environment-configured gateway!"}]
        }
...
def key_validation():
    ...
    def validate_key(key_name: str, api_key: str, provider: str = None) -> Dict[str, Any]:
        ...
        # Simple test payload
        payload = {
            "model": "gpt-4o-mini" if not provider or provider == "openai" else "gemini-2.0-flash-exp",
    ...
    results["openai_client"] = validate_key("openai_client", keys_to_test["openai_client"], "openai")
    results["gemini_client"] = validate_key("gemini_client", keys_to_test["gemini_client"], "gemini")
...
def main():
    ...
    print("These examples demonstrate different authentication modes and key management.")
    print("Make sure to configure your API keys in the script or environment variables.\n")
    
    # Display configuration
    print("Current Configuration:")
    print(f"- Gateway URL: {GATEWAY_BASE_URL}")
    print(f"- Gateway Key: {'Set' if GATEWAY_API_KEY != 'your-gateway-api-key-here' else 'Not configured'}")
    print(f"- OpenAI Client Key: {'Set' if CLIENT_OPENAI_KEY != 'your-openai-api-key-here' else 'Not configured'}")
    print(f"- Gemini Client Key: {'Set' if CLIENT_GEMINI_KEY != 'your-gemini-api-key-here' else 'Not configured'}")
    print()
